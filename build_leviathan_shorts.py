...
os.makedirs("temp_leviathan", exist_ok=True)
...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="WREST - ALL INSTRUMENTS & VOCALS", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "LEVIATHAN", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "THE TENTH SUB LEVEL OF SUICIDE (2003)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "USBM TITAN • 1998 - PRESENT", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n--- 1. Gerando Slides Verticais de Leviathan ---")
    cover = "assets/leviathan/album_cover.jpg"
    wrest = "assets/leviathan/wrest_photo.jpg"

    create_vertical_slide(wrest, "temp_leviathan/slide_intro.jpg", is_intro=True)
    create_vertical_slide(cover, "temp_leviathan/slide_track1.jpg", header="THE TENTH SUB LEVEL OF SUICIDE", subheader="LEVIATHAN (2003)", now_playing="Sardoniscorn")
    create_vertical_slide(cover, "temp_leviathan/slide_track2.jpg", header="THE TENTH SUB LEVEL OF SUICIDE", subheader="LEVIATHAN (2003)", now_playing="The Bitter Emblem of Dissolve")
    create_vertical_slide(cover, "temp_leviathan/slide_track3.jpg", header="THE TENTH SUB LEVEL OF SUICIDE", subheader="LEVIATHAN (2003)", now_playing="The Idiot Sun")
    create_vertical_slide(wrest, "temp_leviathan/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="LEVIATHAN • WREST", is_outro=True)

    print("\n--- 2. Processando Áudio com Ducking Perfeito ---")
    intro_voice = "temp_leviathan/intro.mp3"
    intro_dur = get_audio_duration(intro_voice) + 0.3
    intro_audio = "temp_leviathan/intro_padded.mp3"
    subprocess.run([
    ...
    t1_audio, t1_dur = build_perfect_ducked_audio(
        "temp_leviathan/track1.mp3",
        "temp_leviathan/sample1_sardoniscorn.mp3",
        "temp_leviathan/t1.mp3",
        solo_duration=10.0
    )

    t2_audio, t2_dur = build_perfect_ducked_audio(
        "temp_leviathan/track2.mp3",
        "temp_leviathan/sample2_bitter_emblem.mp3",
        "temp_leviathan/t2.mp3",
        solo_duration=10.0
    )

    t3_audio, t3_dur = build_perfect_ducked_audio(
        "temp_leviathan/track3.mp3",
        "temp_leviathan/sample3_idiot_sun.mp3",
        "temp_leviathan/t3.mp3",
        solo_duration=10.0
    )

    outro_audio, outro_dur = build_outro_audio(
        "temp_leviathan/outro.mp3",
        "temp_leviathan/sample_ambient.mp3",
        "temp_leviathan/outro_ducked.mp3"
    )

    SEGMENTS = [
        {"img": "temp_leviathan/slide_intro.jpg", "audio": intro_audio, "dur": intro_dur},
        {"img": "temp_leviathan/slide_track1.jpg", "audio": t1_audio, "dur": t1_dur},
        {"img": "temp_leviathan/slide_track2.jpg", "audio": t2_audio, "dur": t2_dur},
        {"img": "temp_leviathan/slide_track3.jpg", "audio": t3_audio, "dur": t3_dur},
        {"img": "temp_leviathan/slide_outro.jpg", "audio": outro_audio, "dur": outro_dur},
    ]

    print("\n--- 3. Renderizando Segmentos em Vídeo ---")
    clip_list_file = "temp_leviathan/clips.txt"
    clip_files = []

    for idx, seg in enumerate(SEGMENTS):
        clip_path = f"temp_leviathan/clip_{idx:02d}.mp4"
        dur = seg["dur"]
    ...
    final_output = "output/02_Leviathan_Shorts_9x16.mp4"
    ...
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #2 GERADO EM: {final_output}")
