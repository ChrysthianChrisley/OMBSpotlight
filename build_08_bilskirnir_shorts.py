...
def generate_voice(text, out_path):
    # Enforce pure edge_tts to ensure identical voice tone and sample rate across all clips
    ...
    # Padroniza áudio da voz estritamente para 48000Hz stereo
    cmd = [
        FFMPEG_PATH, "-y",
        "-i", raw_path,
        "-ar", "48000",
    ...
    if os.path.exists(raw_path):
        try:
            ...
            pass
    print(f"✅ Narração normalizada em 48kHz: {out_path}")
    return out_path
...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="WIDAR - ALL INSTRUMENTS & SONGWRITING", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "BILSKIRNIR", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "WOTANSVOLK (2007)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "PAGAN BLACK METAL • 1996 - PRESENT", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def add_to_upload_queue(item):
    ...
    # Remove qualquer versão antiga do item se já existir
    queue = [q for q in queue if q.get("title") != item.get("title")]
    queue.append(item)
    with open(q_file, "w", encoding="utf-8") as f:
        json.dump(queue, f, indent=2, ensure_ascii=False)
    print(f"Vídeo atualizado na fila de pendências: {item.get('title')}")

def run():
    print("\n=======================================================")
    print("REFAZENDO COM PERFEIÇÃO EPISÓDIO #8: BILSKIRNIR - WOTANSVOLK")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=O7AzgkNfn9M"
    cover = "assets/bilskirnir/album_cover_square.jpg"

    # Limpeza de arquivos de áudio anteriores para garantir consistência
    old_files = [
        "intro.mp3", "track1.mp3", "track2.mp3", "track3.mp3", "outro.mp3",
        "intro_padded.mp3", "t1.mp3", "t2.mp3", "t3.mp3", "outro_ducked.mp3"
    ]
    for of in old_files:
        p = os.path.join(TEMP_DIR, of)
        if os.path.exists(p):
            try:
                os.remove(p)
            except Exception:
    ...
    # 1. Download de áudio e corte de amostras (48kHz)
    ...
    s1 = cut_local_sample(full_audio_file, 50, 60, f"{TEMP_DIR}/sample1_wotansvolk.mp3")
    s2 = cut_local_sample(full_audio_file, 280, 60, f"{TEMP_DIR}/sample2_weltenbrand.mp3")
    s3 = cut_local_sample(full_audio_file, 1080, 60, f"{TEMP_DIR}/sample3_niedergang.mp3")
    s_amb = cut_local_sample(full_audio_file, 1850, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração Consistente (Edge-TTS ChristopherNeural em 48kHz)
    print("\n--- Gerando Narração Consistente ---")
    v_intro = generate_voice(
        "Founded in 1996 in Hesse, Germany, Bilskirnir stands as one of the rawest and most enduring solo projects in underground pagan black metal.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Behind every guitar riff, bassline, and drum beat was sole musician Widar. In 2007, he unleashed the seminal underground album 'Wotansvolk'. Here is the crushing title track.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "With 'Weltenbrand', relentless lo-fi aggression meets solemn archaic melodies, channeling the bleak mysticism of ancient European forests.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "'Niedergang' demonstrates Widar's mastery of somber mid-tempo repetition, forging pure hypnotic atmosphere in complete isolation.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Operating in total autonomy for decades, Widar proved that unyielding artistic vision requires no collective, only singular solitary will.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="WOTANSVOLK", subheader="BILSKIRNIR (2007)", now_playing="Wotansvolk")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="WOTANSVOLK", subheader="BILSKIRNIR (2007)", now_playing="Weltenbrand")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="WOTANSVOLK", subheader="BILSKIRNIR (2007)", now_playing="Niedergang")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="BILSKIRNIR • WIDAR", is_outro=True)
    ...
    final_output = "output/08_Bilskirnir_Shorts_9x16.mp4"
    ...
    post_info_file = "output/08_Bilskirnir_post_info.txt"
    title_text = "#8 - Bilskirnir | One-Man Band Spotlight #Shorts"
...
Behind every note was multi-instrumentalist Widar. Operating in complete autonomy, he wrote, performed, and recorded every instrument himself, releasing the cult masterpiece "Wotansvolk" in 2007.

Widar's unyielding solitary vision and archaic European atmosphere remain an influential monument in underground extreme metal.
...
#shorts #blackmetal #paganblackmetal #onemanband #bilskirnir #widar #wotansvolk #rawblackmetal #metal #undergroundmetal #heavymetal


=== TIKTOK (Legenda curta) ===
#8 - Bilskirnir | One-Man Band Spotlight 🇩🇪 Widar recorded every instrument alone on "Wotansvolk" (2007).

Full Album: {full_album_url}

#blackmetal #paganblackmetal #onemanband #bilskirnir #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "paganblackmetal", "onemanband", "bilskirnir", "widar", "wotansvolk"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #8 REFEITO COM PERFEIÇÃO EM: {final_output}")
