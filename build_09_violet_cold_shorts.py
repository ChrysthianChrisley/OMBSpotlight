...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="EMIN GULIYEV - ALL INSTRUMENTS & PRODUCTION", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "VIOLET COLD", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "ANOMIE (2017)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "AVANT-GARDE BLACKGAZE • 2013 - PRESENT", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #9: VIOLET COLD - ANOMIE")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=9IjPxL9_JP4"
    cover = "assets/violet_cold/album_cover_square.jpg"
    ...
    s1 = cut_local_sample(full_audio_file, 80, 60, f"{TEMP_DIR}/sample1_anomie.mp3")
    s2 = cut_local_sample(full_audio_file, 660, 60, f"{TEMP_DIR}/sample2_devastation.mp3")
    s3 = cut_local_sample(full_audio_file, 1260, 60, f"{TEMP_DIR}/sample3_lovegaze.mp3")
    s_amb = cut_local_sample(full_audio_file, 2700, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração
    print("\n--- Gerando Narração ---")
    v_intro = generate_voice(
        "Founded in 2013 in Baku, Azerbaijan, Violet Cold stands as one of the most visionary and genre-defying solo projects in contemporary extreme metal.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Operating in total isolation, multi-instrumentalist Emin Guliyev merges ferocious blackgaze with euphoric post-rock textures. In 2017, he released his breakthrough opus, 'Anomie'. Here is the title track.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "With 'She Spoke Of Her Devastation', torrential blast beats collide with sweeping electronic synths, unleashing an overwhelming tidal wave of light and sorrow.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "'Lovegaze' shatters traditional metal boundaries entirely, fusing radiant guitar shoegaze and atmospheric warmth into a transcendent sonic sanctuary.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Emin Guliyev redefined the emotional horizons of blackgaze from Azerbaijan. Violet Cold is pure boundless creative freedom.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="ANOMIE", subheader="VIOLET COLD (2017)", now_playing="Anomie")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="ANOMIE", subheader="VIOLET COLD (2017)", now_playing="She Spoke Of Her Devastation")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="ANOMIE", subheader="VIOLET COLD (2017)", now_playing="Lovegaze")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="VIOLET COLD • EMIN GULIYEV", is_outro=True)
    ...
    final_output = "output/09_Violet_Cold_Shorts_9x16.mp4"
    ...
    post_info_file = "output/09_Violet_Cold_post_info.txt"
    title_text = "#9 - Violet Cold | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Founded in 2013 in Baku, Azerbaijan, Violet Cold stands as one of the most visionary and genre-defying solo projects in contemporary extreme music.

Behind every note was sole multi-instrumentalist Emin Guliyev. Operating in complete independence, he wrote, performed, recorded, and produced his monumental 2017 blackgaze masterpiece "Anomie".

Emin Guliyev's breathtaking fusion of euphoric post-rock, cosmic electronics, and searing black metal remains a landmark in modern metal history.

🎧 Featured tracks in this video:
01. Anomie
02. She Spoke Of Her Devastation
03. Lovegaze
...
#shorts #blackmetal #blackgaze #postblackmetal #onemanband #violetcold #eminguliyev #anomie #shoegaze #metal #undergroundmetal


=== TIKTOK (Legenda curta) ===
#9 - Violet Cold | One-Man Band Spotlight 🇦🇿 Emin Guliyev recorded every instrument alone on "Anomie" (2017).

Full Album: {full_album_url}

#blackmetal #blackgaze #onemanband #violetcold #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "blackgaze", "onemanband", "violetcold", "eminguliyev", "anomie"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #9 GERADO EM: {final_output}")
