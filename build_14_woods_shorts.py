...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="D. - FOUNDER, GUITARS, BASS & COMPOSITION", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "WOODS OF DESOLATION", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "TORN BEYOND REASON (2011)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "POST-BLACK METAL • AUSTRALIA", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #14: WOODS OF DESOLATION - TORN BEYOND REASON")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=qthQt8nYYvI"
    cover = "assets/woods_of_desolation/album_cover_square.jpg"
    ...
    # 1. Torn Beyond Reason (0 - 383): sample at 60s
    # 2. Darker Days (383 - 692): sample at 430s
    # 4. The Inevitable End (1111 - 1661): sample at 1160s
    # Ambient outro: November (1661 - 1825): sample at 1670s
    s1 = cut_local_sample(full_audio_file, 60, 60, f"{TEMP_DIR}/sample1_torn.mp3")
    s2 = cut_local_sample(full_audio_file, 430, 60, f"{TEMP_DIR}/sample2_darker_days.mp3")
    s3 = cut_local_sample(full_audio_file, 1160, 60, f"{TEMP_DIR}/sample3_inevitable_end.mp3")
    s_amb = cut_local_sample(full_audio_file, 1670, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração Consistente (Edge-TTS ChristopherNeural em 48kHz)
    print("\n--- Gerando Narração Consistente ---")
    v_intro = generate_voice(
        "Founded in 2005 in Australia, Woods of Desolation stands as one of the most revered and emotionally overwhelming projects in depressive and post-black metal history.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Sole mastermind D. wrote, composed, and recorded all sweeping guitar harmonies and bass in isolation. In 2011, he unleashed 'Torn Beyond Reason'. Here is the monumental title track.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "With 'Darker Days', cascading melodic leads collide with intense driving rhythm, channeling a sublime blend of euphoric catharsis and heartbreaking despair.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "'The Inevitable End' delivers soaring walls of bittersweet sound, demonstrating D.'s rare gift for turning profound solitude into transcendent sonic beauty.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "D. elevated post-black metal into pure emotional poetry. Woods of Desolation remains a solitary landmark in extreme music.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="TORN BEYOND REASON", subheader="WOODS OF DESOLATION (2011)", now_playing="Torn Beyond Reason")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="TORN BEYOND REASON", subheader="WOODS OF DESOLATION (2011)", now_playing="Darker Days")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="TORN BEYOND REASON", subheader="WOODS OF DESOLATION (2011)", now_playing="The Inevitable End")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="WOODS OF DESOLATION • D.", is_outro=True)
    ...
    final_output = "output/14_Woods_Of_Desolation_Shorts_9x16.mp4"
    ...
    post_info_file = "output/14_Woods_Of_Desolation_post_info.txt"
    title_text = "#14 - Woods of Desolation | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Founded in 2005 in Australia, Woods of Desolation stands as one of the most revered and influential post-black metal / DSBM entities in extreme music.

Behind every evocative melody and driving rhythm is sole mastermind and composer D. (also known as Dolor). Writing, arranging, and tracking every guitar and bass layer in solitude, he created the 2011 masterpiece "Torn Beyond Reason".

🎧 Featured tracks in this video:
01. Torn Beyond Reason
02. Darker Days
03. The Inevitable End
...
#shorts #blackmetal #postblackmetal #dsbm #woodsofdesolation #tornbeyondreason #atmosphericblackmetal #onemanband #metal #undergroundmetal


=== TIKTOK (Legenda curta) ===
#14 - Woods of Desolation | One-Man Band Spotlight 🇦🇺 Mastermind D. composed every note on "Torn Beyond Reason" (2011).

Full Album: {full_album_url}

#blackmetal #postblackmetal #onemanband #woodsofdesolation #dsbm #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "postblackmetal", "dsbm", "woodsofdesolation", "onemanband"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #14 GERADO EM: {final_output}")
