...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="AUSTIN LUNN - ALL INSTRUMENTS & VOCALS", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "PANOPTICON", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "AUTUMN ETERNAL (2015)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "ATMOSPHERIC METAL ICON • 2007 - PRESENT", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #5: PANOPTICON - AUTUMN ETERNAL")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=Cy9stVnrSv4"
    cover = "assets/panopticon/album_cover_square.jpg"

    # 1. Download de áudio e corte de amostras musicais
    ...
    s1 = cut_local_sample(full_audio_file, 290, 60, f"{TEMP_DIR}/sample1_north_woods.mp3")
    s2 = cut_local_sample(full_audio_file, 690, 60, f"{TEMP_DIR}/sample2_autumn_eternal.mp3")
    s3 = cut_local_sample(full_audio_file, 2160, 60, f"{TEMP_DIR}/sample3_pale_ghosts.mp3")
    s_amb = cut_local_sample(full_audio_file, 3300, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração ElevenLabs
    print("\n--- Gerando Narração ElevenLabs ---")
    v_intro = generate_voice(
        "Founded in 2007 in Kentucky, Panopticon redefined atmospheric black metal by weaving extreme sorrow with American folk and bluegrass heritage.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Behind every instrument and roaring vocal performance is Austin Lunn. Working in solitary passion, he created his 2015 masterpiece, Autumn Eternal. Here is 'Into the North Woods'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "The title track, 'Autumn Eternal', erupts into majestic melodic cascades, capturing the bittersweet fury and solemn beauty of passing seasons.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "With 'Pale Ghosts', ferocious post-metal intensity collides with haunting atmospheres, demonstrating the staggering creative vision of a solitary artist.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Austin Lunn remains one of the most passionate and authentic creators in modern extreme music. Panopticon is true sonic wilderness.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="AUTUMN ETERNAL", subheader="PANOPTICON (2015)", now_playing="Into the North Woods")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="AUTUMN ETERNAL", subheader="PANOPTICON (2015)", now_playing="Autumn Eternal")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="AUTUMN ETERNAL", subheader="PANOPTICON (2015)", now_playing="Pale Ghosts")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="PANOPTICON • AUSTIN LUNN", is_outro=True)
    ...
    final_output = "output/05_Panopticon_Shorts_9x16.mp4"
    ...
    post_info_file = "output/05_Panopticon_post_info.txt"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
#5 - Panopticon | One-Man Band Spotlight #Shorts


=== DESCRIÇÃO (Copiar e colar) ===
Founded in 2007 in Louisville, Kentucky, Panopticon redefined atmospheric black metal by weaving extreme fury with American folk and bluegrass roots.

Behind every note was multi-instrumentalist Austin Lunn (A. Lunn). Performing drums, guitars, bass, flutes, banjos, and vocals alone in personal immersion, he released the widely acclaimed masterpiece "Autumn Eternal" in 2015.

Austin Lunn's passionate solitary vision remains an enduring monument to natural sorrow, memory, and artistic independence.

🎧 Featured tracks in this video:
01. Into the North Woods
02. Autumn Eternal
03. Pale Ghosts
...
#shorts #blackmetal #atmosphericblackmetal #onemanband #panopticon #austinlunn #autumneternal #folkmetal #metal #undergroundmetal #heavymetal


=== TIKTOK (Legenda curta) ===
#5 - Panopticon | One-Man Band Spotlight 🇺🇸 Austin Lunn recorded every instrument alone on "Autumn Eternal" (2015).

Full Album: {full_album_url}

#blackmetal #atmosphericblackmetal #onemanband #panopticon #austinlunn #metalhead #undergroundmetal #metalmusic #shorts
"""
    ...
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #5 GERADO EM: {final_output}")
