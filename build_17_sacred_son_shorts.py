...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="DANE CROSS - 100% RECORDED SOLO", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "SACRED SON", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "SACRED SON (2017)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "ATMOSPHERIC BLACK METAL • LONDON, UK", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #17: SACRED SON - SACRED SON")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=ttdcFanWF_w"
    cover = "assets/sacred_son/album_cover_square.jpg"
    ...
    # 1. Cleave the Alicorn (0 - 459): sample at 50s
    # 2. Apocalyptic Winter (459 - 932): sample at 520s
    # 4. Sepulchral Ritual (1256 - 1803): sample at 1330s
    # Ambient outro: Ethereal Light (932 - 1256): sample at 960s
    s1 = cut_local_sample(full_audio_file, 50, 60, f"{TEMP_DIR}/sample1_cleave.mp3")
    s2 = cut_local_sample(full_audio_file, 520, 60, f"{TEMP_DIR}/sample2_apocalyptic.mp3")
    s3 = cut_local_sample(full_audio_file, 1330, 60, f"{TEMP_DIR}/sample3_sepulchral.mp3")
    s_amb = cut_local_sample(full_audio_file, 960, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração Consistente (Edge-TTS ChristopherNeural em 48kHz)
    print("\n--- Gerando Narração Consistente ---")
    v_intro = generate_voice(
        "Emerging from the United Kingdom, Sacred Son instantly went viral with its unconventional vacation album art, but behind it lay an extraordinary atmospheric black metal tour de force.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Crucially, the entire self-titled debut was written, performed, programmed, and recorded one hundred percent solo by mastermind Dane Cross. Here is 'Cleave the Alicorn'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "'Apocalyptic Winter' surges with majestic tremolo melodies, sweeping synth pads, and explosive energy, defying black metal stereotypes through sheer artistic authenticity.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "With 'Sepulchral Ritual', expansive post-rock soundscapes and thunderous blast beats intertwine in an uplifting and triumphant sonic climax.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Dane Cross proved that monumental vision needs no grim façade or large ensemble. Sacred Son is pure solitary black metal triumph.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="SACRED SON", subheader="SACRED SON (2017)", now_playing="Cleave the Alicorn")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="SACRED SON", subheader="SACRED SON (2017)", now_playing="Apocalyptic Winter")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="SACRED SON", subheader="SACRED SON (2017)", now_playing="Sepulchral Ritual")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="SACRED SON • DANE CROSS", is_outro=True)
    ...
    final_output = "output/17_Sacred_Son_Shorts_9x16.mp4"
    ...
    post_info_file = "output/17_Sacred_Son_post_info.txt"
    title_text = "#17 - Sacred Son | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Emerging from the United Kingdom, Sacred Son gained international attention with its unconventional holiday beach album cover, contrasting sharply with its deeply evocative and epic atmospheric black metal sound.

Behind the debut album was solitary musician Dane Cross. Operating in complete creative autonomy, Cross wrote, performed, programmed, and recorded every instrument and vocal line one hundred percent alone.

🎧 Featured tracks in this video:
01. Cleave the Alicorn
02. Apocalyptic Winter
03. Sepulchral Ritual
...
#shorts #blackmetal #atmosphericblackmetal #sacredson #danecross #postblackmetal #onemanband #ukmetal #metal #undergroundmetal


=== TIKTOK (Legenda curta) ===
#17 - Sacred Son | One-Man Band Spotlight 🇬🇧 Dane Cross recorded 100% solo on "Sacred Son" (2017).

Full Album: {full_album_url}

#blackmetal #atmosphericblackmetal #onemanband #sacredson #danecross #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "atmosphericblackmetal", "sacredson", "danecross", "onemanband"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #17 GERADO EM: {final_output}")
