...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="FUNEBRE - ALL INSTRUMENTS & VOCALS", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "FUNESTO", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "DEPRESSIVOS HINOS SUICIDAS (2007)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "DEPRESSIVE BLACK METAL • BRAZIL", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #15: FUNESTO - DEPRESSIVOS HINOS SUICIDAS")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=asyt4QXQJvE"
    cover = "assets/funesto/album_cover_square.jpg"
    ...
    # 1. Depressivo Suicidio (0 - 376): sample at 120s
    # 2. Meus Gritos Não São Ouvidos (376 - 743): sample at 430s
    # 3. Sobre Meu Sangue (743 - 1116): sample at 790s
    # Ambient outro: Contemplando a Nostalgia (3409 - 3762): sample at 3450s
    s1 = cut_local_sample(full_audio_file, 120, 60, f"{TEMP_DIR}/sample1_suicidio.mp3")
    s2 = cut_local_sample(full_audio_file, 430, 60, f"{TEMP_DIR}/sample2_gritos.mp3")
    s3 = cut_local_sample(full_audio_file, 790, 60, f"{TEMP_DIR}/sample3_sangue.mp3")
    s_amb = cut_local_sample(full_audio_file, 3450, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração Consistente (Edge-TTS ChristopherNeural em 48kHz)
    print("\n--- Gerando Narração Consistente ---")
    v_intro = generate_voice(
        "Hailing from Brazil, Funesto is one of the rawest and most visceral solitary DSBM projects in South American extreme metal history.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Behind every agonized shriek and hypnotic guitar dirge was sole musician Sergio França, known as Funebre. In 2007, he released 'Depressivos hinos suicidas'. Here is 'Depressivo Suicidio'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "'Meus Gritos Não São Ouvidos' channels relentless walls of agonizing lo-fi distortion and desperate weeping vocals that echo across absolute isolation.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "In 'Sobre Meu Sangue', hypnotic depressive rhythms and bleak minor chords forge a suffocating funeral march into psychological darkness.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Funebre captured genuine psychological devastation in pure raw form. Funesto stands as an uncompromising Brazilian DSBM monument.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="DEPRESSIVOS HINOS SUICIDAS", subheader="FUNESTO (2007)", now_playing="Depressivo Suicidio")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="DEPRESSIVOS HINOS SUICIDAS", subheader="FUNESTO (2007)", now_playing="Meus Gritos Não São Ouvidos")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="DEPRESSIVOS HINOS SUICIDAS", subheader="FUNESTO (2007)", now_playing="Sobre Meu Sangue")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="FUNESTO • FUNEBRE", is_outro=True)
    ...
    final_output = "output/15_Funesto_Shorts_9x16.mp4"
    ...
    post_info_file = "output/15_Funesto_post_info.txt"
    title_text = "#15 - Funesto | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Hailing from Brazil, Funesto stands as one of the rawest and most agonizing solitary DSBM projects in South American extreme metal history.

Behind every harrowing scream, lo-fi tremolo riff, and hypnotic drum beat was sole mastermind Funebre (Sergio França). Operating in complete isolation, he channeled pure psychological grief and desolate despair into the 2007 cult masterpiece "Depressivos hinos suicidas".

🎧 Featured tracks in this video:
01. Depressivo Suicidio
02. Meus Gritos Não São Ouvidos
03. Sobre Meu Sangue
...
#shorts #blackmetal #dsbm #depressiveblackmetal #funesto #funebre #brazilianmetal #onemanband #rawblackmetal #metal #undergroundmetal


=== TIKTOK (Legenda curta) ===
#15 - Funesto | One-Man Band Spotlight 🇧🇷 Funebre recorded every instrument alone on "Depressivos hinos suicidas" (2007).

Full Album: {full_album_url}

#blackmetal #dsbm #onemanband #funesto #brazilianmetal #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "dsbm", "depressiveblackmetal", "funesto", "funebre", "onemanband"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #15 GERADO EM: {final_output}")
