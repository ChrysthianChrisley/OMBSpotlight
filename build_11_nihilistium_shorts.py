...
def generate_voice(text, out_path):
    ...
    print(f"Sintetizando via Edge-TTS (ChristopherNeural): {out_path}...")
...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="MÖRTEMIIS - ALL INSTRUMENTS & VOCALS", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "NIHILISTIUM", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "NIHILISTIUM (2018)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "DEPRESSIVE SUICIDAL BLACK METAL • UK", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #11: NIHILISTIUM - NIHILISTIUM")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=K_-8IpJNly8"
    cover = "assets/nihilistium/album_cover_square.jpg"
    ...
    s1 = cut_local_sample(full_audio_file, 620, 60, f"{TEMP_DIR}/sample1_die_alone.mp3")
    s2 = cut_local_sample(full_audio_file, 1140, 60, f"{TEMP_DIR}/sample2_heartache.mp3")
    s3 = cut_local_sample(full_audio_file, 2430, 60, f"{TEMP_DIR}/sample3_left_arm.mp3")
    s_amb = cut_local_sample(full_audio_file, 2100, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração Consistente (Edge-TTS ChristopherNeural em 48kHz)
    print("\n--- Gerando Narração Consistente ---")
    v_intro = generate_voice(
        "Emerging from the United Kingdom, Nihilistium is one of the most raw and unfiltered solo DSBM projects in modern underground black metal.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Crafted entirely by sole musician Mörtemiis, the self-titled album is an agonizing confession of isolation and emotional wreckage. Here is 'Die Alone'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "'Intense Ceaseless Heartache' unleashes suffocating walls of lo-fi tremolo picking paired with desperate, tortured vocal cries that tear through the silence.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "With 'Left Arm Path', dragging rhythms and bleak melodies form a dark, hypnotic lamentation on despair and mortal fragility.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Mörtemiis channels pure unadulterated suffering through sound. Nihilistium represents the darkest depths of solitary black metal.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="NIHILISTIUM", subheader="NIHILISTIUM (2018)", now_playing="Die Alone")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="NIHILISTIUM", subheader="NIHILISTIUM (2018)", now_playing="Intense Ceaseless Heartache")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="NIHILISTIUM", subheader="NIHILISTIUM (2018)", now_playing="Left Arm Path")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="NIHILISTIUM • MÖRTEMIIS", is_outro=True)
    ...
    final_output = "output/11_Nihilistium_Shorts_9x16.mp4"
    ...
    post_info_file = "output/11_Nihilistium_post_info.txt"
    title_text = "#11 - Nihilistium | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Emerging from the United Kingdom, Nihilistium stands as one of the most raw, agonizing, and poignant solo DSBM projects in modern underground black metal.

Behind this suffocating sonic creation is sole musician Mörtemiis. Operating in total solitude, Mörtemiis performs every howling vocal, bleak guitar chord, and melancholic rhythm, crafting an intimate journey into absolute despair on the self-titled album.

🎧 Featured tracks in this video:
01. Die Alone
02. Intense Ceaseless Heartache
03. Left Arm Path
...
#shorts #blackmetal #dsbm #depressiveblackmetal #onemanband #nihilistium #mortemiis #postblackmetal #undergroundmetal #ambientblackmetal


=== TIKTOK (Legenda curta) ===
#11 - Nihilistium | One-Man Band Spotlight 🇬🇧 Mörtemiis recorded every instrument alone on "Nihilistium" (2018).

Full Album: {full_album_url}

#blackmetal #dsbm #onemanband #nihilistium #undergroundmetal #metalhead #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "dsbm", "depressiveblackmetal", "onemanband", "nihilistium", "mortemiis"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #11 GERADO EM: {final_output}")
