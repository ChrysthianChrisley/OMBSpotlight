...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="HERBST - ALL INSTRUMENTS & VOCALS", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "HERBSTREGEN", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "LEBEN GESCHEITERT (2007)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "GERMAN DSBM CULT • 2007 - PRESENT", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #7: HERBSTREGEN - LEBEN GESCHEITERT")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=RrL6DuvHeZQ"
    cover = "assets/herbstregen/album_cover_square.jpg"
    ...
    s1 = cut_local_sample(full_audio_file, 60, 60, f"{TEMP_DIR}/sample1_gemaeuer.mp3")
    s2 = cut_local_sample(full_audio_file, 420, 60, f"{TEMP_DIR}/sample2_life_failed.mp3")
    s3 = cut_local_sample(full_audio_file, 900, 60, f"{TEMP_DIR}/sample3_traenen.mp3")
    s_amb = cut_local_sample(full_audio_file, 2410, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração ElevenLabs
    print("\n--- Gerando Narração ElevenLabs ---")
    v_intro = generate_voice(
        "Founded in Germany, Herbstregen emerged as one of the rawest and most sincere expressions of depressive black metal.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Behind every instrument was sole creator Herbst, also known as Inferno. Pouring private agony into music, he crafted the cult 2007 album, Leben Gescheitert. Here is 'Dieses alte Gemäuer'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "With 'Life Failed', somber clean melodies blend with searing distortion and anguished screams, laying bare pure vulnerability.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "'Verzweifelt rinnen Tränen' delivers crushing mid-tempo despair, echoing the solitary desolation that defines underground German DSBM.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "Herbstregen stands as a monument of unfiltered sorrow in depressive metal. A testament to genuine grief created in utter isolation.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="LEBEN GESCHEITERT", subheader="HERBSTREGEN (2007)", now_playing="Dieses alte Gemäuer")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="LEBEN GESCHEITERT", subheader="HERBSTREGEN (2007)", now_playing="Life Failed")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="LEBEN GESCHEITERT", subheader="HERBSTREGEN (2007)", now_playing="Verzweifelt rinnen Tränen")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="HERBSTREGEN • HERBST", is_outro=True)
    ...
    final_output = "output/07_Herbstregen_Shorts_9x16.mp4"
    ...
    post_info_file = "output/07_Herbstregen_post_info.txt"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
#7 - Herbstregen | One-Man Band Spotlight #Shorts


=== DESCRIÇÃO (Copiar e colar) ===
Founded in Germany, Herbstregen emerged as one of the rawest and most sincere expressions of depressive black metal.

Behind every note was solitary creator Herbst (also known as Inferno). Composing, recording, and screaming in total isolation, he created the cult DSBM opus "Leben Gescheitert" in 2007.

Herbstregen's piercing mournful riffs and vulnerable acoustic melancholia remain an enduring underground testament to solitary sorrow.

🎧 Featured tracks in this video:
01. Dieses alte Gemäuer
02. Life Failed
03. Verzweifelt rinnen Tränen
...
#shorts #blackmetal #dsbm #depressiveblackmetal #onemanband #herbstregen #lebengescheitert #germandsbm #metal #undergroundmetal #heavymetal


=== TIKTOK (Legenda curta) ===
#7 - Herbstregen | One-Man Band Spotlight 🇩🇪 Herbst recorded every instrument alone on "Leben Gescheitert" (2007).

Full Album: {full_album_url}

#blackmetal #dsbm #onemanband #herbstregen #metalhead #undergroundmetal #shorts
"""
    ...
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #7 GERADO EM: {final_output}")
