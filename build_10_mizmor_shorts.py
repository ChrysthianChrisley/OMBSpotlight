...
def generate_voice(text, out_path, voice_id="JBFqnCBsd6RMkjVDRZzb"):
    ...
    if API_KEY:
        try:
            print(f"Tentando ElevenLabs: {out_path} ({len(clean_text)} chars)...")
            ...
            with open(out_path, "wb") as f:
                for chunk in audio_stream:
                    f.write(chunk)
            print(f"✅ Narração salva via ElevenLabs: {out_path}")
            return out_path
        except Exception as e:
            print(f"⚠️ ElevenLabs indisponível ({e}). Usando voz alternativa Christopher Neural...")
    
    print(f"Sintetizando via Edge-TTS (en-US-ChristopherNeural): {out_path}...")
    ...
    print(f"✅ Narração salva via Edge-TTS: {out_path}")
    return out_path
...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="A.L.N. - ALL INSTRUMENTS & COMPOSITION", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "MIZMOR (מזמור)", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "YODH (2016)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "BLACKENED DOOM • 2012 - PRESENT", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def add_to_upload_queue(item):
    ...
    if os.path.exists(q_file):
        try:
            ...
            queue = []
    if not any(q.get("title") == item.get("title") for q in queue):
        queue.append(item)
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #10: MIZMOR - YODH")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=Obd52q2vQ8M"
    cover = "assets/mizmor/album_cover_square.jpg"
    ...
    # 1. Woe Regains My Substance (0 - 797): sampling intense blackened doom section at 150s
    # 2. A Semblance Waning (797 - 1511): sampling crushing blast/screams at 880s
    # 3. The Serpent Eats Its Tail (1511 - 2390): sampling massive funeral riff at 1650s
    # Ambient outro: from Bask In The Lingering around 3100s
    s1 = cut_local_sample(full_audio_file, 150, 60, f"{TEMP_DIR}/sample1_woe.mp3")
    s2 = cut_local_sample(full_audio_file, 880, 60, f"{TEMP_DIR}/sample2_semblance.mp3")
    s3 = cut_local_sample(full_audio_file, 1650, 60, f"{TEMP_DIR}/sample3_serpent.mp3")
    s_amb = cut_local_sample(full_audio_file, 3100, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração
    print("\n--- Gerando Narração ---")
    v_intro = generate_voice(
        "Founded in 2012 in Portland, Oregon, Mizmor stands as a towering monument of blackened doom and existential dread in modern extreme music.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Behind the suffocating sonic weight is solitary mastermind A.L.N., who performs, records, and produces every single instrument alone. In 2016, he unveiled 'Yodh'. Here is 'Woe Regains My Substance'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "'A Semblance Waning' collapses funeral doom gravity into ferocious black metal shrieks, chronicling spiritual disintegration and relentless agony.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "In 'The Serpent Eats Its Tail', cavernous guitar drones and monolithic drumming weave a hypnotic descent into complete nihilistic darkness.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "A.L.N. pushed the limits of solo doom and black metal to their absolute emotional peak. Mizmor remains an unparalleled sonic catharsis.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="YODH", subheader="MIZMOR (2016)", now_playing="I. Woe Regains My Substance")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="YODH", subheader="MIZMOR (2016)", now_playing="II. A Semblance Waning")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="YODH", subheader="MIZMOR (2016)", now_playing="III. The Serpent Eats Its Tail")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="MIZMOR • A.L.N.", is_outro=True)
    ...
    final_output = "output/10_Mizmor_Shorts_9x16.mp4"
    ...
    post_info_file = "output/10_Mizmor_post_info.txt"
    title_text = "#10 - Mizmor | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Founded in 2012 in Portland, Oregon, Mizmor (מזמור) stands as a towering monument of blackened doom and existential dread in modern extreme music.

Behind the suffocating sonic weight is solitary mastermind A.L.N. Operating in complete isolation, he wrote, performed, recorded, and mixed every instrument and vocal line himself on his monumental 2016 masterpiece "Yodh".

Adorned with the haunting artwork of Zdzisław Beksiński, "Yodh" remains one of the most intense and cathartic artistic statements in underground metal history.
...
#shorts #blackmetal #doommetal #sludgemetal #blackeneddoom #onemanband #mizmor #aln #yodh #dronemetal #metal #undergroundmetal


=== TIKTOK (Legenda curta) ===
#10 - Mizmor | One-Man Band Spotlight 🇺🇸 A.L.N. recorded every instrument alone on "Yodh" (2016).

Full Album: {full_album_url}

#blackmetal #doommetal #onemanband #mizmor #sludgemetal #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "doommetal", "blackeneddoom", "onemanband", "mizmor", "aln", "yodh"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #10 GERADO EM: {final_output}")
