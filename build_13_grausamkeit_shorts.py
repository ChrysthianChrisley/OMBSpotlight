...
def create_vertical_slide(fg_path, out_path, header="ONE-MAN BAND SPOTLIGHT", subheader="B.S.O.D. - ALL INSTRUMENTS & VOCALS", now_playing=None, is_intro=False, is_outro=False):
    W, H = 1080, 1920
    ...
    if is_intro:
        ...
        draw.text((W // 2, footer_y + 42), "GRAUSAMKEIT", font=font_band, fill=(255, 255, 255), anchor="mm")
        draw.text((W // 2, footer_y + 88), "STARDUST (1999)", font=font_album, fill=(200, 200, 200), anchor="mm")
    elif now_playing:
        ...
        draw.text((W // 2, footer_y + 48), "RAW BLACK METAL • DUNGEON SYNTH • GERMANY", font=font_now_single, fill=(220, 220, 220), anchor="mm")
...
def run():
    print("\n=======================================================")
    print("PRODUZINDO EPISÓDIO #13: GRAUSAMKEIT - STARDUST")
    print("=======================================================")

    full_album_url = "https://www.youtube.com/watch?v=ZaCmKuyQlQw"
    cover = "assets/grausamkeit/album_cover_square.jpg"
    ...
    # 2. PCP Jesus (starts 106): sample at 120s
    # 3. Stardust (starts 277): sample at 300s
    # 4. Something About Spacebase (starts 441): sample at 470s
    # Ambient outro: Outro (starts 827) -> sample at 835s
    s1 = cut_local_sample(full_audio_file, 120, 60, f"{TEMP_DIR}/sample1_pcp_jesus.mp3")
    s2 = cut_local_sample(full_audio_file, 300, 60, f"{TEMP_DIR}/sample2_stardust.mp3")
    s3 = cut_local_sample(full_audio_file, 470, 60, f"{TEMP_DIR}/sample3_spacebase.mp3")
    s_amb = cut_local_sample(full_audio_file, 835, 50, f"{TEMP_DIR}/sample_ambient.mp3")

    # 2. Narração Consistente (Edge-TTS ChristopherNeural em 48kHz)
    print("\n--- Gerando Narração Consistente ---")
    v_intro = generate_voice(
        "Formed in 1994 in Germany, Grausamkeit is the notorious and cult lo-fi raw black metal project born from the chaotic mind of B.S.o.D.",
        f"{TEMP_DIR}/intro.mp3"
    )
    v_t1 = generate_voice(
        "Sole mastermind Andreas Bettinger recorded all harsh guitars, screeching synthesizers, and drum machines alone in his bedroom. In 1999, he released 'Stardust'. Here is 'PCP Jesus'.",
        f"{TEMP_DIR}/track1.mp3"
    )
    v_t2 = generate_voice(
        "The title track 'Stardust' blends eerie dungeon synth leads with frantic venomous black metal, creating a feverish, drug-fueled nightmare unique to underground metal.",
        f"{TEMP_DIR}/track2.mp3"
    )
    v_t3 = generate_voice(
        "With 'Something About Spacebase', lo-fi tape hiss and deranged melodic hooks forge an irresistible hypnotic aura that gained an obsessive worldwide cult following.",
        f"{TEMP_DIR}/track3.mp3"
    )
    v_outro = generate_voice(
        "B.S.o.D. broke every convention of traditional black metal. Grausamkeit remains an indelible, bizarre cornerstone of outsider extreme art.",
    ...
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track1.jpg", header="STARDUST", subheader="GRAUSAMKEIT (1999)", now_playing="PCP Jesus")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track2.jpg", header="STARDUST", subheader="GRAUSAMKEIT (1999)", now_playing="Stardust (Midtro)")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_track3.jpg", header="STARDUST", subheader="GRAUSAMKEIT (1999)", now_playing="Something About Spacebase")
    create_vertical_slide(cover, f"{TEMP_DIR}/slide_outro.jpg", header="ONE-MAN BAND SPOTLIGHT", subheader="GRAUSAMKEIT • B.S.O.D.", is_outro=True)
    ...
    final_output = "output/13_Grausamkeit_Shorts_9x16.mp4"
    ...
    post_info_file = "output/13_Grausamkeit_post_info.txt"
    title_text = "#13 - Grausamkeit | One-Man Band Spotlight #Shorts"
    post_content = f"""=== TÍTULO (Copiar e colar) ===
{title_text}


=== DESCRIÇÃO (Copiar e colar) ===
Formed in 1994 in Germany, Grausamkeit is the notorious and iconic lo-fi raw black metal project forged by sole mastermind B.S.o.D. (Andreas Bettinger).

Operating in complete bedroom isolation, Bettinger recorded every demented guitar riff, screeching synth line, and abrasive vocal scream alone onto tape, creating the 1999 underground cult classic "Stardust".

🎧 Featured tracks in this video:
01. PCP Jesus
02. Stardust (Midtro "Jill's Death")
03. Something About Spacebase (The Crack Session)

💿 Listen to the full demo on YouTube:
{full_album_url}

Subscribe for more retrospective spotlights on iconic One-Man Bands!

#shorts #blackmetal #rawblackmetal #dungeonsynth #grausamkeit #bsod #andreasbettinger #stardust #lofiblackmetal #metal #undergroundmetal


=== TIKTOK (Legenda curta) ===
#13 - Grausamkeit | One-Man Band Spotlight 🇩🇪 B.S.o.D. recorded every instrument alone on "Stardust" (1999).

Full Album: {full_album_url}

#blackmetal #rawblackmetal #onemanband #grausamkeit #bsod #metalhead #undergroundmetal #shorts
"""
    ...
    add_to_upload_queue({
        ...
        "status": "pending",
        "video_url": None,
        "tags": ["shorts", "blackmetal", "rawblackmetal", "dungeonsynth", "onemanband", "grausamkeit", "bsod"]
    })

    total_sec = sum(s["dur"] for s in SEGMENTS)
    print(f"\n=======================================================")
    print(f"SUCESSO! EPISÓDIO #13 GERADO EM: {final_output}")
